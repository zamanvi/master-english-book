...
// International's own sidebar - separate component from Sidebar.jsx
// (Bangladesh's), same structure but links under /international/book/
// and English-only copy.
export default function InternationalSidebar({ chaptersWithContent }) {
      ...
      <nav className="flex-1 overflow-y-auto py-2 pb-16">
        ...
        {filtered.length === 0 && search && (
          <p className="px-4 py-10 text-center text-slate-500 text-sm font-hind">
            No lessons found for &ldquo;{search}&rdquo;
          </p>
        )}

        {chaptersWithContent?.length === 0 && !search && (
          <p className="px-4 py-10 text-center text-slate-500 text-sm font-hind">
            Content coming soon.
          </p>
        )}
